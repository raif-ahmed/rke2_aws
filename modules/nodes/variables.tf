variable "availability_zones" {
  type        = list(string)
  description = "List of the availability zones in which to create the nodes. The length of this list must match instance_count."
}
...
variable "node_type" {
  type        = string
  description = "The node type to be identified in the ec2 name, it can be either master or agent."

  validation {
    condition     = can(regex("^(master|agent)$", var.node_type))
    error_message = "For the node_type value only master and agent are allowed."
  }
}